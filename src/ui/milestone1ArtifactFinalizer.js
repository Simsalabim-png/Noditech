...
function appScriptBounds(html) {const marker='function App()';const m=html.indexOf(marker);if(m<0)throw new Error('build identity: function App() not found');if(html.indexOf(marker,m+marker.length)>=0)throw new Error('build identity: function App() not unique');const open=html.lastIndexOf('<script',m);if(open<0)throw new Error('build identity: app <script> open not found');const openEnd=html.indexOf('>',open)+1;const close=html.indexOf('</script>',m);if(close<0)throw new Error('build identity: app </script> close not found');return{start:openEnd,end:close};}
function applyBuildIdentity(html,identity=BUILD_IDENTITY){let out=replaceOnce(html,'const BUILD_VERSION = "Build 9.6-rc8";',`const BUILD_VERSION = "${identity.version}";`,'BUILD_VERSION');out=replaceOnce(out,'const BUILD_DATE = "2026-06-25";',`const BUILD_DATE = "${identity.date}";`,'BUILD_DATE');out=replaceOnce(out,'const BUILD_HASH = "b6ebc906e926";','const BUILD_HASH = "";','BUILD_HASH blank');const{start,end}=appScriptBounds(out);const hash=sha256(out.slice(start,end)).slice(0,12);out=replaceOnce(out,'const BUILD_HASH = "";',`const BUILD_HASH = "${hash}";`,'BUILD_HASH inject');return{html:out,version:identity.version,date:identity.date,hash};}
function applyMilestone1ArtifactFinalizer(html){if(typeof html!=='string'||!html.includes('function useCanonicalTemperature(initialC, unit)'))throw new Error('Milestone 1 transformed artifact required');let out=applyRefrigerantProjection(html);out=applyAirLiquidRanges(out);out=applyLiquidLiquidRanges(out);out=applyAirAirFinalGuards(out);out=applyMilestone1Hooks(out);const identity=applyBuildIdentity(out);out=identity.html;const after={airAir:sha256(section(out,'function AirAir({','function AirLiquid({')),airLiquid:sha256(section(out,'function AirLiquid({','function LiqLiq({'))};return{html:out,after,sha256:sha256(out),identity:{version:identity.version,date:identity.date,hash:identity.hash}};}
module.exports={applyMilestone1ArtifactFinalizer,applyMilestone1Hooks,applyBuildIdentity,appScriptBounds,BUILD_IDENTITY,applyAirAirFinalGuards,applyAirLiquidRanges,applyLiquidLiquidRanges,applyRefrigerantProjection,projectRange,replaceOnce,sha256};
