...
const { prodSha, source, sourceSha }=ex.extract();
let compiled;
...
if(/<\/script\s*>/i.test(compiled)) bad.push('contains </script>');
if(/<script[\s>]/i.test(compiled)) bad.push('contains <script>');
...
const header='/* GENERATED — do not edit. Build-time compile of the extracted Step 3 application source.\n'
 +'   Compiler: '+LOCK.compiler+'@'+LOCK.version+' (preset react). Source SHA-256: '+sourceSha+'.\n'
...
const equiv={ generated:process.env.BUILD_TS||new Date().toISOString(),
  ...
  extracted_application_source_sha256:sourceSha,
  ...
  command:'node tests/compile_app.js',
  guards_passed:true,
  equivalence_status:'EQUIVALENT — app.compiled.js is the build-time compilation of the byte-identical application source extracted from the packaged production artifact' };
...
console.log('compiled ->', path.relative(ROOT,OUT), '| source_sha', sourceSha.slice(0,12), '| compiled_sha', compiledSha.slice(0,12));
