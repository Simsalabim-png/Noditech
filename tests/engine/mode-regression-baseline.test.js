...
for(const [index,c] of baseline.liquidLiquidCases.entries()) test(`L/L baseline ${c.id}`,()=>{const x=current(c.input),p=physical(c.input,c.physicalReference);near(x.qc,c.currentCalculator.QcoldKW);near(x.qh,c.currentCalculator.QhotKW);near(x.cop,c.currentCalculator.copCooling);near(x.bal,c.currentCalculator.displayedBalancePct);assert.equal(x.classification,c.currentCalculator.classification);assert.equal(p.valid,c.physicalReference.directionValid);near(p.exp,c.physicalReference.expectedHotKW);near(p.res,c.physicalReference.residualKW);near(p.dev,c.physicalReference.deviationPct);if(c.physicalReference.QcoldKW!=null)near(p.qc,c.physicalReference.QcoldKW);assert.equal(index>=0,true);});
