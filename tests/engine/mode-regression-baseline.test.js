...
function physical(input,ref){const cp=ref.cpKJkgK??4.18,rho=ref.rhoKgL??1,dc=input.cold.TinC-input.cold.ToutC,dh=input.hot.ToutC-input.hot.TinC,qc=cp*input.cold.flowLs*rho*dc,qh=cp*input.hot.flowLs*rho*dh,exp=qc+input.powerKW,res=qh-qc-input.powerKW;return{valid:dc>0&&dh>0,exp,res,dev:(res/exp)*100};}

for(const index of [0,1]) test(`L/L baseline ${baseline.liquidLiquidCases[index].id}`,()=>{const c=baseline.liquidLiquidCases[index],x=current(c.input),p=physical(c.input,c.physicalReference);near(x.qc,c.currentCalculator.QcoldKW);near(x.qh,c.currentCalculator.QhotKW);near(x.cop,c.currentCalculator.copCooling);near(x.bal,c.currentCalculator.displayedBalancePct);assert.equal(x.classification,c.currentCalculator.classification);assert.equal(p.valid,c.physicalReference.directionValid);near(p.exp,c.physicalReference.expectedHotKW);near(p.res,c.physicalReference.residualKW);near(p.dev,c.physicalReference.deviationPct);});
