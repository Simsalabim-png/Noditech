...
for(const index of [0,1]) test(`L/L baseline case ${index+1}`,()=>{const c=baseline.liquidLiquidCases[index],x=current(c.input),p=physical(c.input,c.physicalReference);near(x.qc,c.currentCalculator.QcoldKW);near(x.qh,c.currentCalculator.QhotKW);near(x.cop,c.currentCalculator.copCooling);near(x.bal,c.currentCalculator.displayedBalancePct);assert.equal(x.classification,c.currentCalculator.classification);assert.equal(p.valid,c.physicalReference.directionValid);near(p.exp,c.physicalReference.expectedHotKW);near(p.res,c.physicalReference.residualKW);near(p.dev,c.physicalReference.deviationPct);});
